...
from model.model_iiw import ImplicitGuideDPT

from iiw_dataset import IIWDataset
...
def train(train_loader, model, optimizer, epoch, print_freq=1,guide_model=None):
    ...
    for i, (input, point_pair, pair_label, name, guide) in enumerate(train_loader):
        ...
        output, guide_output = model(input_var,input_guide)
        
        loss_array = list()
        bs = output[0].shape[0]
        for j in range(bs):
            reflectance_map = output[0][j].permute(1,2,0)
        ...
        # scores_array = list()
        # for i in range(bs):
        #     reflectance_map = output[0][i].permute(1,2,0)
...
def validate(val_loader, model, print_freq=10, guide_model=None, epoch=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    losses_array = list()
    for it in task_list:
    ...
    for i, (input, point_pair, pair_label, name,guide) in enumerate(val_loader):
        with torch.no_grad():
            ...
            output, guide_output = model(input_var,input_guide, name)
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar',guide=False):
    if not guide:
        ...
        torch.save(state, filename)
        if is_best:
            shutil.copyfile(filename, 'guide_model_best.pth.tar')
...
def train_iiw(args):
    ...
    if len(task_list) == 1:
        single_model = ImplicitGuideDPT(args.classes, backbone="vitb_rn50_384",guide=True)
    else:
        pass
    ...
    train_loader = torch.utils.data.DataLoader(
        IIWDataset(data_dir= data_dir, split = 'train',transforms = transforms.Compose(naive_t)), #t
    ...
    val_loader = torch.utils.data.DataLoader(
            IIWDataset(data_dir=data_dir,split= 'train',transforms= transforms.Compose(naive_t)),
    ...
    if args.evaluate:
        validate(val_loader, model, epoch=0, guide_model=guide_model)
        return
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        train(train_loader, model, optimizer, epoch,guide_model=guide_model)
        prec1 = validate(val_loader, model, epoch=epoch,guide_model=guide_model)
        ...
        if (epoch + 1) % 10 == 0:
            ...
            if guide_model is not None:
                history_path = 'guide_checkpoint_{:03d}.pth.tar'.format(epoch + 1)
                shutil.copyfile('guide_checkpoint_latest.pth.tar', history_path)
