...
class JIIF(nn.Module):
    ...
    def __init__(self, num_classes, feat_dim=128, guide_dim=128,mlp_dim=[512,256,128,64]): #
        ...
        #self.lr_feat_proj = nn.Conv2d(feat_dim,guide_dim,1)
    ...
    def forward(self, image, lr_res, res, lr_image_feature, lr_image):
        bs = image.shape[0]
        ...
        # lr_guide = lr_image_feature#self.lr_feat_proj(lr_image_feature) #,self.image_encoder(lr_image)
...
class ImplicitGuideDPT(torch.nn.Module):
    def __init__(self, num_classes, guide = True, **kwargs):
        super(ImplicitGuideDPT, self).__init__()
        self.dpt = DPTSegmentationModel(num_classes, backbone="vitb_rn50_384")
        self.guide = guide
        if self.guide:
            self.guide_model = JIIF(num_classes)

    def forward(self, x, input_guide=None, lr_image=None):
        output,lr_map,lr_feature = self.dpt(x)

        guide_output = None
        if self.guide:
            guide_output = self.guide_model(input_guide,lr_map[0],output[0],lr_feature[0], lr_image)
